...
pub fn list_wallpapers(conn: &Connection, favourite_only: bool) -> rusqlite::Result<Vec<serde_json::Value>> {
    let sql = if favourite_only {
        "SELECT key, name, type, thumb, thumb_sm, favourite, hue, sat, tags, colors, matugen, video_file, we_id, analyzed_by, filesize, width, height, mtime, richness, apply_count FROM meta WHERE favourite = 1 ORDER BY name"
    } else {
        "SELECT key, name, type, thumb, thumb_sm, favourite, hue, sat, tags, colors, matugen, video_file, we_id, analyzed_by, filesize, width, height, mtime, richness, apply_count FROM meta ORDER BY name"
    };
    ...
    let rows = stmt.query_map([], |row| {
        Ok(serde_json::json!({
            ...
            "richness": row.get::<_, Option<i64>>(18)?,
            "apply_count": row.get::<_, Option<i64>>(19)?,
        }))
    })?;
...
pub fn update_analysis(
    ...
    let changed = conn.execute(
        "UPDATE meta SET tags = COALESCE(?1, tags), tags_raw = COALESCE(?1, tags_raw),
         ...
         hue = COALESCE(?4, hue), sat = COALESCE(?5, sat)
         WHERE key = ?6",
        params![tags, colors, analyzed_by, hue, sat, key],
    )?;
    Ok(changed > 0)
}
...
mod tests {
    ...
    fn update_analysis_coalesces_none_fields() {
        let conn = mem();
        insert(&conn, "a", "static", "a");
        assert!(update_analysis(&conn, "a", Some("tag1,tag2"), None, Some("manual"), Some(120), None).unwrap());
        let rows = list_wallpapers(&conn, false).unwrap();
        assert_eq!(rows[0]["tags"], "tag1,tag2");
        assert_eq!(rows[0]["analyzed_by"], "manual");
        assert_eq!(rows[0]["hue"], 120);
        assert_eq!(rows[0]["sat"], 0);

        update_analysis(&conn, "a", None, Some("#fff"), None, None, None).unwrap();
        let rows = list_wallpapers(&conn, false).unwrap();
        assert_eq!(rows[0]["tags"], "tag1,tag2");
